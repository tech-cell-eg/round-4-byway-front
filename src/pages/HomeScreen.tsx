      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center gap-8 md:flex-row md:justify-between ">
          <div className="flex flex-col space-y-6 md:w-1/2 ">
            <h1 className="text-4xl font-bold text-gray-600 dark:text-white sm:text-5xl">
...
<<<<<<< HEAD
      ...
      <div className="w-full bg-gray-100 dark:bg-gray-800 px-6 py-8 mt-5 md:px-10 lg:px-20 lg:py-10 flex flex-col lg:flex-row justify-between items-center gap-10 lg:gap-6">
=======
      {/* Status */}
...
>>>>>>> 96f2c506f47291b343a5d6ff3a537613a571de71
        {stats.map((item, index) => (
          ...
          >
            <div className="text-center px-4">
              <h3 className="text-2xl font-bold text-black dark:text-white">{item.number}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-300">{item.label}</p>
            </div>
            {index !== stats.length - 1 && (
              <div className="hidden lg:block h-12 w-px bg-gray-300 dark:bg-gray-600 mx-2" />
            )}
          </div>
        ))}
      </div>
      ...
      <div className="w-full max-w-[1280px] mx-auto px-4 py-6 pt-16">
        ...
        <div className=" grid grid-cols-2 lg:grid-cols-4 gap-4 ">
          {categories.map((cat, idx) => (
            <div
              key={idx}
              className="flex flex-col items-center justify-center text-center border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 p-6 shadow-sm hover:shadow-md transition"
            >
              <div className="text-blue-500 mb-3">{cat.icon}</div>
              <h3 className="font-semibold text-base dark:text-white">{cat.title}</h3>
              <p className="text-sm text-gray-500 dark:text-gray-300">{cat.count} Courses</p>
            </div>
          ))}
        </div>
      </div>
        ...
        {instructors.map((instructor, index) => (
          ...
          >
            <div className="flex flex-col space-y-6 md:w-1/2">
              ...
              <p className="text-lg text-gray-600 dark:text-gray-300">{instructor.description}</p>
              <button  className="flex items-center gap-3 w-fit rounded-lg bg-gray-900 dark:bg-gray-700 px-6 py-3 text-white transition-colors hover:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 cursor-pointer">
               <Link to="/checkout"> {instructor.buttonText}</Link>
                <FaArrowRight />
              </button>
            </div>
            <div className="md:w-1/2">
